...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @tailrec
    def go(prev2: Int, prev1: Int, i: Int): Int = {
      if (i == 0) prev2
      else go(prev1, prev1 + prev2, i-1)
    }
    go(0, 1, n)
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    @tailrec
    def isSortedRec(index: Int): Boolean = {
      if (as.size - index < 2) true
      else gt(as(index), as(index+1)) && isSortedRec(index + 1)
    }
    isSortedRec(0)
  }
